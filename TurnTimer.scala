...
object TurnTimer:
  ...
  def update(tt: TurnTimer) : Option[TurnTimer] = 
    val t2 = math.round(System.currentTimeMillis / 100).toInt // this is 10ths of a second
    if (t2 > tt.iThisTurnTime)
      Some(tt.copy(iThisTurnTime = t2))
    else
      None
    end if
  end update
  ...
  def show( model: FlicFlacGameModel) : SceneUpdateFragment = 
    ...
    val iSliderXPos = (math.round((GameAssets.GameSceneDimensions.right - model.hexBoard3.pBase.x - 120)*dSF)).toInt + model.hexBoard3.pBase.x
